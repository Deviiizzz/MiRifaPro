...
import fondoCliente from './fondo-cliente.jpeg';
            ...
            {cart.length > 0 && selectedRifa.estado === 'activa' && (
              <div className="fixed bottom-8 left-4 right-4 bg-black text-white p-7 rounded-[3rem] flex justify-between items-center shadow-[0_20px_50px_rgba(230,0,0,0.2)] z-40 border-2 border-zinc-800">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em]">{cart.length} Seleccionados</p>
                        <div className="flex items-end gap-2">
                            <p className="text-3xl font-black italic tracking-tighter">${(cart.length * selectedRifa.precio).toFixed(2)}</p>
                            {tasaBcv && <p className="text-xs font-bold text-zinc-400 mb-1">/ {(cart.length * selectedRifa.precio * tasaBcv).toFixed(2)} Bs</p>}
                        </div>
                    </div>
                </div>
            ...
            {!paymentMethod ? (
                ...
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                    <div className="bg-red-600 p-6 rounded-[2.5rem] mb-6 text-white text-center shadow-xl shadow-red-200 border-2 border-red-800">
                        ...
                        <p className="text-sm font-black leading-relaxed mb-3">BANCO CENTRAL<br/>V-12.345.678<br/>0412-0000000</p>
                        <div className="bg-red-700/50 p-3 rounded-2xl border border-red-500 backdrop-blur-sm">
                            <p className="text-[10px] font-black uppercase text-red-200 mb-1">Monto a pagar</p>
                            <p className="text-xl font-black">${(cart.length * selectedRifa.precio).toFixed(2)} USD</p>
                            {tasaBcv && <p className="text-sm font-black text-yellow-300">{(cart.length * selectedRifa.precio * tasaBcv).toFixed(2)} Bs</p>}
                        </div>
                    </div>
